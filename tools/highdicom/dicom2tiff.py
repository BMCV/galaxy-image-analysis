...
def dicom_to_tiff(
    ...
    # Set defaults
    config = dict(config)
    config.setdefault('dtype', np.float32)  # highdicom default is float64
    config.setdefault('apply_voi_transform', False)

    # If the image is a tiled image, ...
    if hasattr(dcm, 'TotalPixelMatrixRows') and hasattr(dcm, 'TotalPixelMatrixColumns'):
